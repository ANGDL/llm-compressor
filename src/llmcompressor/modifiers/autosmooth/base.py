...
class AutoSmoothModifier(Modifier, QuantizationMixin):
        ...
        for mapping in self.mappings:
            ...
            for smooth_layers, *nested_balance_layers in match_modules_set(
                ...
                if len(smooth_layers) > 1:
                    raise ValueError(
                        "Smooth needs to match a single smoothlayer for each mapping but "
                ...
                if skip_message:
                    logger.warning(
                        f"skipping Smooth for {smooth_name} for mapping {mapping}"
                        + skip_message
                    )
    ...
    def _compute_best_scale(
        ...
        logger.debug(
            f"Smooth grid search for {mapping.smooth_name}: "
