...
_DISABLE_OFFSET_NORM_CALIBRATION = (
    os.getenv("DISABLE_OFFSET_NORM_CALIBRATION", "0") == "1"
)
...
class CalibrationOffsetNorm(NormCalibrationModule):
    ...
    def __init__(self, original: torch.nn.Module, config):
        super().__init__()
        original_weight: Any = getattr(original, "weight")
        self.eps = getattr(original, "eps")
        self._orig_dtype = original_weight.dtype
        self.weight = torch.nn.Parameter(
            (1.0 + original_weight.data.float()).to(original_weight.dtype)
        )
    ...
    def restore(self, original: torch.nn.Module) -> torch.nn.Module:
        original_weight: Any = getattr(original, "weight")
        original_weight.data = (self.weight.data.float() - 1.0).to(self._orig_dtype)
        return original
...
def norm_calibration_context(model: PreTrainedModel):
    ...
    if _DISABLE_OFFSET_NORM_CALIBRATION:
        logger.info("Offset norm calibration disabled by DISABLE_OFFSET_NORM_CALIBRATION=1")
        yield
        return
...
def _is_registered(name: str, subclass: type[RegistryMixin]):
    lookup = standardize_lookup_name(name)
    return lookup in subclass.registered_names() or lookup in subclass.registered_aliases()
